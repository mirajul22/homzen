...
export default function Cards() {
  return (
    <div className="flex flex-wrap gap-10 ">
      {CardsData.map((data) => (
        <div className=" block hover:overflow-hidden w-[400px] h-[435px]  border-2 rounded-3xl">
...
function CartOurServices() {
  return (
    <div className="flex gap-10 m-10 ">
      {services.map((d) => (
        <div className=" text-center p-10 border-2 w-[400px] rounded-3xl hover:bg-[#d1cfcf]">
          <p className="text-6xl px-20 py-5">
            <span>
              <FcHome className=" mx-14" />
            </span>
          </p>
          <h1 className="text-2xl font-bold p-2">{d.title}</h1>
          <p className="p-2">{d.description}</p>
          <button
            className=" m-4 font-bold border-2 border-black py-2 px-4 hover:bg-red-500 hover:text-white"
            type="button"
          >
            {d.text}
          </button>
...
function CartChoose() {
  return (
    <div className=" cursor-pointer">
      {services2.map((d) => (
        <div className="flex shadow-2xl hover:scale-110 hover:ease-linear hover:duration-300 m-10 py-4 px-2 gap-4 border-2 w-[500px] rounded-3xl">
          <p className="">
            <span>
...
function CartSubscription(){
  return(
    <div className="bg-gray-100 w-[300px] h-[480px] hover:duration-300 p-4 rounded-3xl hover:text-white hover:bg-red-500 " >
        <div className="flex">
        ...
        <p className=" flex items-center gap-2 font-bold py-2"><FaCheckCircle className=" text-red-500" /> Transportation for you</p>
        <p className=" flex items-center gap-2 font-bold py-2"><FaCheckCircle className=" text-red-500" /> A hotel or homestay</p>
        <p className=" flex items-center gap-2 font-bold py-2"><FaCheckCircle className=" text-red-500" /> Free meals and drinks</p>
        <p className=" flex items-center gap-2 font-bold py-2"><FaCheckCircle className=" text-red-500" /> 24/7 Support</p>
        <p className=" flex items-center gap-2 font-bold py-2"><FaCheckCircle className=" text-red-500" /> Ultimate users</p>
        <div>
          <button className="font-bold border-2 hover:border-none  py-2 px-10 my-4 border-black bg-white text-black " type="button">Choose The Package</button>
        </div>
    </div>
  )
}
